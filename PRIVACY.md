...
| Télémétrie d'usage | **Rien n'est émis** tant que les trois conditions suivantes ne sont pas réunies : la **politique d'entreprise** de votre organisation — poussée par votre navigateur, hors de portée de tout dépôt — déclare `telemetry.enabled` **et** un point de collecte `https:`, **et** vous avez coché la case dédiée dans la page d'options. Cette case affiche l'adresse, et **votre accord ne vaut que pour celle-là**. Le consentement est stocké **sur cet appareil** (`chrome.storage.local`) et **ne se synchronise pas** ; le décocher désarme immédiatement les onglets déjà ouverts, et reste possible même si la politique a cessé de déclarer un point de collecte. | **Seulement si vous l'avez explicitement autorisée**, et alors uniquement vers le point de collecte de votre organisation. Ce qui part est un **agrégat périodique** : le dépôt affiché (`hôte/portée`), le mode de la configuration, et des compteurs d'identifiants — labels employés, codes de diagnostic, chaînes de sélecteurs dégradées. **Jamais de texte libre** : une valeur qui n'est pas un identifiant de forme attendue est abandonnée plutôt qu'envoyée. La requête ne joint **ni cookie ni référent** — l'adresse de la page de revue ne part donc pas avec elle, ce que `npm run check:beacon` mesure dans un vrai navigateur à chaque exécution. |
| Journal local des dégradations d'interface | `chrome.storage.local`, sur votre appareil : **une ligne par règle de sélection**, cinquante au plus, et chacune ne garde que le **dernier** relevé de sa règle — son nom, l'horodatage, et **l'adresse de la page** telle qu'elle était à cet instant, paramètres de requête et fragment compris. Jamais le contenu de la page. Sur un serveur d'entreprise, ce chemin nomme un dépôt et un numéro de revue : c'est le prix du diagnostic, et la page d'options **efface ce journal d'un clic**. Écrit **toujours**, y compris sans télémétrie — c'est l'outil de diagnostic du §9.4. | Non — il ne quitte votre appareil que si vous avez autorisé la télémétrie ci-dessus, et ce qui part alors ne porte que le **nom de la règle**, jamais l'adresse. |
...
| Usage telemetry | **Nothing is emitted** unless all three of the following hold: your organization's **enterprise policy** — pushed by your browser, out of reach of any repository — declares `telemetry.enabled` **and** an `https:` collection endpoint, **and** you have ticked the dedicated checkbox on the extension's options page. That checkbox shows the address, and **your consent applies to that one only**. Consent is stored **on this device** (`chrome.storage.local`) and **does not sync**; unticking it disarms already-open tabs immediately, and stays possible even if the policy has stopped declaring an endpoint. | **Only if you have explicitly allowed it**, and then only to your organization's endpoint. What leaves is a **periodic aggregate**: the displayed repository (`host/scope`), the configuration's mode, and counters of identifiers — labels used, diagnostic codes, degraded selector chains. **Never free text**: a value that is not a well-formed identifier is dropped rather than sent. The request carries **neither cookies nor a referrer** — the review page's address does not travel with it, which `npm run check:beacon` measures in a real browser on every run. |
| Local journal of interface degradations | `chrome.storage.local`, on your device: **one line per selector rule**, fifty at most, each keeping only that rule's **most recent** observation — its name, a timestamp, and **the page's address** as it stood at that moment, query parameters and fragment included. Never the page's content. On an enterprise server that path names a repository and a review number: this is the privacy trade-off diagnostics require, and the options page **clears this journal in one click**. Written **always**, telemetry or not — it is the §9.4 diagnostic tool. | No — it leaves your device only if you allowed telemetry above, and what leaves then carries only the **rule's name**, never the address. |
