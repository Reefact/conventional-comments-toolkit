...
| Local journal of interface degradations | `chrome.storage.local`, on your device: the **last 50** times the extension failed to recognise an element of the page — the selector rule's name, a timestamp, and **the page's address** as it stood at that moment, query parameters and fragment included. Never the page's content. On an enterprise server that path names a repository and a review number: that is what the diagnostic costs, and the options page **clears this journal in one click**. Written **always**, telemetry or not — it is the §9.4 diagnostic tool. | No — it leaves your device only if you allowed telemetry above, and what leaves then carries only the **rule's name**, never the address. |
