...
| Journal local des dégradations d'interface | `chrome.storage.local`, sur votre appareil : les **50 dernières** fois où l'extension n'a pas su reconnaître un élément de la page (nom de la règle de sélection et horodatage, jamais de contenu). Écrit **toujours**, y compris sans télémétrie — c'est l'outil de diagnostic du §9.4, consultable dans la page d'options. | Non — il ne quitte votre appareil que si vous avez autorisé la télémétrie ci-dessus, et alors sous forme de compteurs agrégés. |
...
| Local journal of interface degradations | `chrome.storage.local`, on your device: the **last 50** times the extension failed to recognise an element of the page (the selector rule's name and a timestamp, never any content). Written **always**, telemetry or not — it is the §9.4 diagnostic tool, readable on the options page. | No — it leaves your device only if you allowed telemetry above, and then as aggregated counters. |
