...
function sampleResult() {
  ...
  const result = evaluate({
    pr: GH_PR,
    platform: { id: 'github', suggestionInfoString: 'suggestion', slashCommands: false, commandPrefixes: [] },
    threads: [],
    loose: [],
    config,
    configNotices: [],
...
describe('Annexe B — adaptateur serveur Azure DevOps', () => {
  ...
  it('profil : pas d’étage 0 (info string non établie), pas de commande slash native (§B.6)', () => {
    ...
    expect(adapter.platformProfile()).toEqual({ id: 'azdo', suggestionInfoString: null, slashCommands: false, commandPrefixes: [] });
  });
