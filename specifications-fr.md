...
- il commence par une **commande adressée à un outil** — commande slash (`/azp run`, `/retest`) ou interpellation d'un compte de robot (`@codex review`, `@dependabot rebase`). Les préfixes reconnus sont propres à chaque plateforme, donnés en annexe et portés par `PlatformProfile.commandPrefixes` (§9.2) ; la reconnaissance porte sur le **premier jeton du corps**, qui doit être suivi d'une espace ou de la fin de ligne — `@codex review` est une commande, `@codexplique pourquoi` n'en est pas une ;
...
**La liste des préfixes est fermée, et elle n'est pas configurable.** Pour la raison qui interdit déjà un motif `^/…` dans les valeurs par défaut (§8.2) : un préfixe générique — `^/`, et plus encore `^@\w+` — offrirait un contournement en deux caractères. Le second est le pire des deux, `@` étant aussi la façon d'interpeller une personne : `@alice peux-tu regarder ça ?` est une remarque de revue, et l'exempter viderait la convention de son objet sur la zone où elle sert le plus. Les préfixes reconnus sont donc **nommés un par un** en annexe et portés par l'adaptateur, hors d'atteinte de la configuration d'un dépôt. Un robot **interne**, absent de cette liste, relève de `allowlistPatterns` (§8.2) sous la forme d'un motif ancré et exact — `^@notre-bot\b` —, sous la responsabilité du dépôt qui l'ajoute.

Le contournement que cette liste ouvre — écrire `/retest en fait ce nom est ambigu` — est de la même nature que celui, assumé, du bloc délimité ci-dessous, et il appelle la même réponse : il n'atteint que le critère 1, l'hygiène de forme. Un commentaire exempté n'ouvre aucun fil bloquant, donc n'exempte personne de traiter quoi que ce soit.
...
Le champ `exemptUsers` de l'exemple est un espace réservé : les identités de comptes de service diffèrent d'une plateforme à l'autre et sont données dans les annexes. Aucun motif exemptant un préfixe de commande — `^/…`, et pas davantage `^@\w+` — ne figure dans les valeurs par défaut : le premier permettrait de se soustraire à la contrainte en tapant `/x` devant sa phrase, et le second exempterait en outre l'interpellation d'un collègue, qui est une remarque de revue. Les commandes légitimes, slash comme mentions de robot, relèvent de l'exemption structurelle du §4.2 — liste fermée portée par l'adaptateur —, pas d'un motif configurable. Un robot **interne** reste ajoutable par le dépôt, mais sous la forme d'un motif ancré et exact (`^@notre-bot\b`), qui n'ouvre aucun préfixe générique.
...
interface PlatformProfile {
  ...
  commandPrefixes: string[];           // §4.2 — préfixes de commande reconnus : commandes slash (`/azp`)
                                       // et robots interpellés par mention (`@codex`). Un seul champ, la
                                       // règle de reconnaissance étant la même : premier jeton du corps,
                                       // suivi d'une espace ou de la fin de ligne. Liste **fermée**, portée
                                       // par l'adaptateur, donc hors d'atteinte de la configuration d'un
                                       // dépôt — sans quoi elle serait elle-même un contournement (§4.2, §8.2)
}
...
- **Commandes adressées à un outil** (§4.2). GitHub en connaît deux familles, que `commandPrefixes` (§9.2) porte dans une seule liste :
  - **Préfixe slash** — `/rebase` ; les *comment triggers* d'Azure Pipelines `/azp`, fonctionnalité **GitHub uniquement** qui n'existe pas sur Azure Repos ; les commandes Prow, répandues dans l'écosystème CNCF : `/lgtm`, `/approve`, `/hold`, `/retest`, `/test`, `/override`, `/assign`, `/cc`, `/kind`, `/area`, `/close`, `/reopen`, `/milestone`.
  - **Interpellation d'un compte de robot** — `@dependabot` (`rebase`, `recreate`, `merge`, `squash and merge`, `cancel merge`, `reopen`, `close`, `ignore this major version`…), `@copilot`, `@coderabbitai` (`review`, `full review`, `resolve`, `pause`, `resume`, `ignore`, `configuration`, `help`), `@codex`, `@claude`, `@mergifyio` (`refresh`, `update`, `rebase`, `backport <branche>`, `queue`), `@renovate`, `@rustbot`, `@bors`. C'est la famille la plus employée, et celle dont l'absence coûterait le plus cher : sans elle, `@codex review` tombe en `E-NO-LABEL` et le mode `enforce` **empêche de relancer un relecteur automatique** — la zone de conversation générale, où ces commandes s'écrivent, est validée en format (§4.1).

  Les identités de robot citées ici le sont comme **valeurs**, sujettes à l'évolution de l'écosystème : la liste s'actualise avec l'adaptateur, jamais avec la configuration d'un dépôt (§4.2). `bors r+` et `r? @alice`, qui ne portent **ni slash ni mention**, n'y figurent pas et ne peuvent pas y figurer : `bors` seul exempterait « bors est cassé depuis hier ». Un dépôt qui les emploie les déclare en `allowlistPatterns` sous forme ancrée et exacte — `^bors\s+(r\+|r=|try|retry|ping|delegate)` —, ce que les bornes du §8.2 autorisent.
...
- **Aucune commande native, ni slash ni mention.** Les *comment triggers* du type `/azp run` sont une fonctionnalité d'Azure Pipelines réservée aux dépôts **GitHub** ; Azure Repos passe par les policies de branche. Aucun écosystème de robots pilotés au commentaire ne s'y est établi non plus : la revue de code assistée que la plateforme propose se déclenche par son interface, pas par une interpellation dans un fil. `commandPrefixes` (§9.2) est donc **vide** sur cette plateforme, et l'exemption du §4.2 n'y a pas d'illustration native — elle reste disponible pour les outils tiers éventuellement installés, dont les préfixes s'ajouteraient ici.
