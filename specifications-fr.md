...
**« Vérificateur » nomme un rôle, pas un déploiement.** Le composant B est défini par ce qu'il **fait** — relire l'état courant de la PR, appeler `core/`, publier un statut —, jamais par la machine qui l'exécute. Ce que ce rôle exige de son **support d'exécution** est énuméré au §6.4.1 ; **quel** support le remplit est une décision par plateforme, prise en annexe. Sur GitHub, c'est une **GitHub Action** : elle s'exécute sur l'infrastructure de la plateforme, avec le jeton que celle-ci fournit, et **ne demande aucun service à héberger** (§A.8).

La distinction n'est pas de vocabulaire. Une rédaction antérieure de ce document nommait ce composant « compagnon serveur » et en tirait, page après page, qu'un service devait être déployé, exposé en HTTPS et exploité pour que le blocage de merge existe. C'était confondre le rôle avec l'un de ses supports possibles — celui qui coûte le plus cher à l'organisation qui adopte l'outil, et qui n'est nécessaire sur aucune des deux plateformes pour la même raison. **O3 n'exige pas un serveur ; il exige un vérificateur.**
...
- Un bandeau en tête de PR récapitule : *N **fils** bloquants non résolus*, avec liens d'ancrage vers chacun. Des **fils**, et non des commentaires : c'est ce que compte le critère 2 (§6.2.1) et ce que porte `unresolvedBlockingCount`, et `CA-03` exige que les trois décomptes concordent — en valeur comme en sens. **Sa source est le résumé publié par le composant B** (`readPublishedResult()`, §9.2.3) dès qu'il est présent sur la page : c'est ce qui garantit que le bandeau et le check comptent la même chose (`CA-03`). Le **décompte** vient toujours de ce résumé, qui fait autorité ; les **liens d'ancrage** viennent du DOM de la page, qui porte les fils — un fil dont l'état n'y est pas rendu (`resolution: 'unknown'`) recevant quand même son ancre, puisque rien n'est compté ici.
  ...
  **Les deux divergent, et c'est assumé.** Trois règles du document garantissent que l'extension ne peut pas retrouver localement l'ensemble exact des fils que le vérificateur compte : une **résolution refusée** (§6.1) laisse le fil marqué *Resolved* dans la page ; une **édition affaiblissante** (§6.1) affiche `note:` là où le vérificateur maintient `issue:` ; et l'**épinglage** (§8.1.3) fait juger le vérificateur sur une configuration que l'extension n'a plus. Vouloir la concordance exacte imposerait à l'extension de trancher des autorisations, ce que le §10 lui interdit.
  ...
  Le bandeau affiche donc **le décompte publié** comme titre, et les ancres qu'il a su apparier. Quand il en apparie moins, il l'indique — « 2 sur 3 localisés » — et renvoie au statut pour le détail. `CA-03` porte sur le décompte affiché, jamais sur le nombre d'ancres. En l'absence de résumé — composant B non activé sur le dépôt, ou première évaluation encore en cours — il se rabat sur les fils lus dans le DOM (`getThreads()`), un fil dont l'état n'y est pas rendu (`resolution: 'unknown'`) étant compté **non résolu**, comme au §B.5 ; il indique alors explicitement qu'il s'agit d'une vue locale et non de l'état de conformité.
...
#### 6.2.4 Rapport à blanc, PR en brouillon, et silence du vérificateur
...
**Le profil A ajoute un piège, et un seul : désactiver le workflow n'est pas un retour arrière.** Il est le geste qui vient à l'esprit, et il produit exactement ce que l'avant-dernier point interdit — plus aucun statut publié, donc toutes les PR bloquées par un check obligatoire qui n'arrivera jamais. Le repli est le même qu'ailleurs : **passer en `warn`**, qui continue de publier du vert. Désactiver le workflow ne vient qu'après le retrait de la protection, jamais avant.
...
#### 6.4.1 Ce que le vérificateur exige de son support d'exécution

Le reste du §6.4 énonce des **mécanismes**. Ils servent **cinq besoins**, et cinq seulement :

| # | Besoin | Pourquoi il existe |
|:-:|--------|--------------------|
| 1 | **Être déclenché** quand l'état de la PR change | un statut qu'aucun changement ne réveille est faux dès le premier commentaire |
| 2 | **Relire l'état courant** de la PR et la configuration du dépôt | §6.4, « relire l'état courant » — jamais le contenu de l'événement |
| 3 | **Empêcher qu'une évaluation ancienne écrase une plus récente** | rien ne garantit l'ordre, ni entre webhooks ni entre exécutions concurrentes |
| 4 | **Retenir trois faits d'une évaluation à la suivante** — la configuration épinglée (§8.1.3), le verdict de première observation et l'ensemble des fils bloquants observés (§6.1) | ces trois-là **seuls** ne se déduisent pas de l'état courant : ils portent sur un passé que la plateforme n'expose plus |
| 5 | **Publier** un statut que la protection de branche sait exiger | c'est O3 |

Un **support d'exécution** est ce qui remplit ces cinq besoins. Le document en reconnaît deux et **n'en privilégie aucun par principe** : le bon support est le plus léger que la plateforme rende possible, et cela se tranche plateforme par plateforme, en annexe.

**Profil A — natif plateforme.** Le vérificateur s'exécute dans l'intégration continue de la plateforme, déclenché par ses événements, authentifié par le jeton qu'elle fournit à l'exécution. **Rien à héberger, rien à exposer, rien à exploiter.** C'est le profil de GitHub (§A.8) : une GitHub Action. Les besoins 1, 3 et 5 y sont remplis par la plateforme ; le besoin 4 l'est en **écrivant l'état dans l'objet publié**, que la plateforme conserve — le vérificateur ne possède alors aucun stockage.

**Profil B — service hébergé.** Le vérificateur est un service que l'organisation déploie, expose en HTTPS et exploite. Il souscrit aux webhooks, tient son propre stockage et son propre ordonnancement. C'est le profil d'Azure DevOps (§B.7), faute d'équivalent au couple « déclencheur de revue + jeton d'écriture » que GitHub fournit sans rien demander.

**Tout ce qui suit dans le §6.4 est écrit pour le profil B**, qui doit tout construire lui-même. Chaque mécanisme y est **la réponse à l'un des cinq besoins, jamais un besoin de plus** : un support qui satisfait le besoin autrement n'a pas à le reproduire.

| Mécanisme du §6.4 | Besoin servi | Ce qu'il devient en profil A |
|---|:-:|---|
| Réconciliation périodique | 1 | facultative, et sa nécessité se juge sur ce que la plateforme n'émet pas (§A.8) |
| Fenêtre de coalescence (`server.coalesceWindowSeconds`) | 3 | l'annulation des exécutions concurrentes de la plateforme la rend sans objet |
| Numéro de séquence par PR | 3 | conservé comme **règle**, pas comme compteur : le statut publié porte le marqueur de récence de l'exécution qui l'a écrit, et une exécution refuse d'écraser un statut plus récent que le sien |
| Stockage propre au vérificateur | 4 | remplacé par l'état porté par le statut publié |
| Délai de grâce et `degradedSince` | — | remplacé : une lecture impossible est réessayée dans l'exécution, puis laisse l'exécution en échec sans rien publier. Le recours n'est plus la procédure du §6.3.3 mais le **rejeu de l'exécution**, à un clic et sans administrateur — ce qui est précisément ce que le délai de grâce existait pour éviter |
| Point d'entrée d'administration (§6.2.4) | — | remplacé par un déclenchement manuel paramétré, soumis aux mêmes habilitations |

**Ce dont le profil A ne dispense pas.** La relecture de l'état courant avant publication, la publication sur le SHA de tête relu juste avant l'appel, l'idempotence, et **l'intégralité des règles des §6.1 à §6.3** valent quel que soit le support. Ce sont des règles de **jugement** ; seuls les mécanismes de transport et de persistance dépendent du support. Un vérificateur qui allégerait le jugement au motif qu'il s'exécute dans un runner ne serait pas un profil, il serait faux.

**Un besoin 4 réduit à trois objets, et pourquoi les dix autres tombent.** La liste « Stockage » plus bas énumère treize objets pour le profil B. Dix d'entre eux n'existent que parce qu'un service hébergé doit reconstruire ce que la plateforme lui cache ou ne lui garantit pas : les compteurs de séquence, le dernier résultat publié, `degradedSince`, la dernière configuration effective, le drapeau « déjà évalué », l'exemption active là où la provenance des étiquettes n'est pas lisible. Un support qui **lit le statut qu'il a lui-même publié** les retrouve tous sans rien posséder. Les trois qui restent au besoin 4 sont d'une autre nature : ils portent sur des **corps de commentaires qui n'existent plus** — une racine éditée, un `E-CONFLICT` corrigé —, et aucune API ne les rend. C'est la seule persistance que ce document impose, sur l'une ou l'autre plateforme.

#### 6.4.2 Séquence d'évaluation
...
« Actif » ne veut pas dire « hébergé » : sur GitHub, activer le composant B, c'est ajouter un fichier de workflow et déclarer le check obligatoire (§A.8). Ce que cette section exige, c'est qu'**un** vérificateur publie un statut sur cette PR, jamais qu'un service tourne quelque part.
...
¹ *La colonne « PR bloquée » suppose le composant B actif sur le dépôt (§6) — ce qui, sur GitHub, ne demande aucun hébergement (§6.4.1, §A.8). Sans lui, le mode `enforce` ne bloque que l'envoi de commentaires non conformes côté extension (composant A) — rien n'empêche la complétion de la PR.*
...
| **B — Vérificateur** | **Profil A** (§6.4.1) : le workflow **imposé au niveau de l'organisation** — *ruleset* exigeant un workflow défini dans un dépôt d'administration —, et l'URL de plancher qu'il porte. **Profil B** : configuration d'installation du service — variable d'environnement, secret de déploiement, ou fichier de configuration dans un dépôt d'administration protégé | L'organisation entière |

**Un workflow versionné dans le dépôt qu'il protège n'est pas un canal de plancher.** C'est le piège propre au profil A, et il faut le nommer : un dépôt qui peut éditer son workflow peut éditer le plancher que ce workflow porte, ce qui n'est plus un plancher mais une valeur par défaut. Le canal n'existe que si le document de plancher est **hors de portée du dépôt** — workflow imposé par une *ruleset* d'organisation, ou document servi par une URL que l'organisation contrôle. Un dépôt qui copie le workflow chez lui obtient l'outil, jamais la contrainte.
...
**Quand elle est écrite.** La configuration épinglée d'une PR est posée à sa **première évaluation** par le composant B, et **jamais réécrite** ensuite — même règle, et même raison, que le verdict de première observation du §6.1 : une valeur réécrite à chaque tour suivrait les durcissements successifs et n'épinglerait plus rien. Une PR ouverte avant l'activation du composant B est donc épinglée sur la configuration en vigueur au jour de sa première évaluation, et non à sa création : c'est la seule date que le service puisse connaître.
...
**2. Une seule autorité de résolution.** L'épinglage est un mécanisme du composant B seul : **le composant A n'épingle pas**. Il n'a pas d'état persistant par PR, et le §10 lui interdit l'appel d'API par lequel il obtiendrait la configuration épinglée. Sur une PR ouverte avant un changement restrictif, il juge donc avec la configuration **courante** pendant que le vérificateur juge avec la configuration **épinglée**.
...
Quand aucun résultat n'est encore publié sur la PR — première évaluation en cours, ou composant B non activé —, il n'y a pas d'empreinte à comparer : l'extension applique le mode configuré, comme partout ailleurs.
...
Elle vise une asymétrie précise. Si l'extension est en avance sur le vérificateur — l'organisation vient d'ajouter un label, l'extension l'a déjà, le vérificateur pas encore — l'utilisateur écrit un commentaire que l'outil vient de l'aider à composer et le check le signale comme non conforme : c'est le sens de divergence qui dégrade une PR. Dans l'autre sens — extension en retard — elle bloque une saisie que le vérificateur accepterait, ce qui est agaçant mais se résout en rechargeant la page. On ne paie donc le coût du rafraîchissement que sur le sens gênant, et seulement au moment d'un rejet, c'est-à-dire rarement.
...
| `server.*` — **portée** | Ces quatre clés décrivent des mécanismes du **profil B** (§6.4.1). En profil A, elles sont **sans objet** et ignorées sans avertissement : la plateforme rend les mécanismes qu'elles règlent, et une valeur y serait un réglage sans effet — ce qui est pire qu'une absence. `server.reconcileIntervalSeconds` fait exception : un profil A qui active la réconciliation périodique la lit (§A.8.2). |
...
├── action/          # Composant B, PROFIL A (§6.4.1) — GitHub Action : lecture de l'état,
│                    # évaluation, publication du check run. Aucun stockage, aucun service (§A.8)
└── server/          # Composant B, PROFIL B (§6.4.1) — service hébergé, pour les plateformes
    ├── compliance/  # sans support natif. Aujourd'hui : Azure DevOps seul (§B.7)
    └── adapters/
        └── azdo/
```

`packages/core/` est publié comme paquet et consommé à l'identique par l'extension et par le vérificateur, **quel que soit le support d'exécution de celui-ci**. **Aucune règle de validation ne doit être dupliquée.**

**Les deux supports ne partagent que `core/`, et c'est délibéré.** Ils appliquent le même jugement — mêmes règles, mêmes types du §9.2.1, même `evaluate()` — mais pas la même orchestration : l'un a besoin d'un ordonnanceur, d'un stockage et d'un serveur HTTP, l'autre reçoit de sa plateforme tout cela. Factoriser leur orchestration ferait porter au support léger le coût du support lourd, ce qui inverserait la raison d'être du profil A. La frontière est celle du §6.4.1 : ce qui **juge** est commun, ce qui **transporte et persiste** ne l'est pas.
...
- Côté vérificateur : **< 60 s** entre l'action d'un utilisateur et un statut à jour sur la PR — **pour les actions que la plateforme notifie**. Sur GitHub, la résolution d'un fil n'en fait pas partie (§A.8.2) : le seuil ne s'y applique pas, et l'y étendre produirait une exigence que rien ne peut tenir. Ce que le §A.8.2 borne à sa place, ce sont les conséquences de ce trou.
- Disponibilité du composant B : cible **99 %** mensuelle. En **profil B** (§6.4.1), avec le délai de grâce du §6.4 comme comportement dégradé, et c'est un engagement que l'organisation prend et mesure. En **profil A**, la disponibilité est celle de l'intégration continue de la plateforme : elle ne s'exploite pas, elle ne se mesure pas séparément, et c'est exactement ce que ce profil échange contre l'absence d'hébergement.
...
| **P5** | Vérificateur (composant B) + status checks — **profil A** sur GitHub (§A.8), **profil B** sur Azure DevOps (§B.7) | P1 |
...
Sur **GitHub**, aucun prérequis d'hébergement ne s'ajoute : c'est tout l'objet du profil A. Un seul choix reste à faire, et il doit être fait **explicitement plutôt que par défaut** :

6. **La parade retenue pour la fenêtre de dé-résolution est choisie** (§A.8.2) — réconciliation planifiée, *Require conversation resolution before merging*, ou aucune. « Aucune » est une réponse admissible ; ne pas avoir posé la question ne l'est pas.
7. **Le workflow compagnon est en place si le dépôt accepte des PR de forks** (§A.8.2). Sans lui, ces PR reçoivent bien un statut — `pull_request_target` s'en charge à l'ouverture et à chaque push —, mais il **cesse d'être rafraîchi dès qu'un relecteur écrit** : un `issue:` bloquant posté après le dernier push ne rougit pas le check. C'est le sens de divergence dangereux, et il ne se voit pas.
...
**Profil retenu : natif plateforme (§6.4.1). Aucun serveur, aucun hébergement, aucun secret à gérer.** Le vérificateur est une **GitHub Action**, exécutée dans le dépôt qu'elle protège, authentifiée par le `GITHUB_TOKEN` que le runner reçoit, et qui publie un *check run* nommé `conventional-comments`. La mise en place tient en un fichier de workflow et une règle de dépôt.

Ce qui suit énonce les **faits de plateforme** dont ce choix dépend. Chacun se vérifie dans la documentation de GitHub, et ce sont eux — non une préférence d'architecture — qui décident. Deux d'entre eux vont contre l'intuition, et c'est précisément pour cela qu'ils sont écrits ici plutôt que supposés.

#### A.8.1 Les cinq besoins du §6.4.1, remplis par GitHub

| Besoin | Ce que GitHub fournit |
|:-:|---|
| 1 — déclenchement | les événements de workflow `pull_request_target`, `pull_request_review`, `pull_request_review_comment` et `issue_comment`, plus `workflow_dispatch` et, au choix de l'organisation, `schedule` |
| 2 — relecture | GraphQL pour les fils et leur état de résolution (§A.6), REST pour les commentaires hors fil, les étiquettes et leur provenance, l'état brouillon et le SHA de tête, la route `contents` pour le fichier de configuration (§A.4) |
| 3 — ordonnancement | `concurrency` groupé par PR avec `cancel-in-progress`, complété par la règle de récence portée par le statut lui-même (§6.4.1) |
| 4 — mémoire | les trois objets du besoin 4 sont écrits **dans le check run publié** et relus à l'exécution suivante — GitHub conserve, le vérificateur ne possède rien |
| 5 — publication | un *check run*, déclarable **required** dans la protection de branche ou dans une *ruleset* |

**Où vit l'état, précisément.** Le corps du *check run* porte, après la sortie humaine, un bloc que GitHub ne rend pas et que l'exécution suivante relit : configuration épinglée (§8.1.3), verdicts de première observation et ensemble des fils bloquants observés (§6.1). L'exécution suivante retrouve le dernier *check run* `conventional-comments` de la PR en remontant ses commits, du plus récent au plus ancien, et s'arrête au premier trouvé. C'est ce qui remplace le stockage du §6.4 : la durabilité est celle de GitHub, la propriété aussi.

#### A.8.2 Trois limites de la plateforme, et ce qu'elles coûtent

**1. La résolution d'un fil ne déclenche aucun workflow.** `pull_request_review_thread` (`resolved` / `unresolved`) existe comme **webhook** — de dépôt, d'organisation et d'App — mais ne figure **pas** parmi les événements déclencheurs de workflows. Or c'est le seul événement qui notifie une résolution.

La conséquence est **asymétrique**, et c'est ce qui la rend acceptable :

- **résoudre** le dernier fil bloquant ne fait pas passer le check au vert de lui-même : il reste **rouge** jusqu'au prochain déclencheur. Gênant, jamais dangereux — la PR reste bloquée alors qu'elle pourrait passer, et le déblocage tient à un rejeu ;
- **dé-résoudre** un fil bloquant ne le fait pas repasser au rouge : il reste **vert** jusqu'au prochain déclencheur. C'est le seul cas où ce profil est moins strict qu'un service abonné aux webhooks, et il doit être décidé les yeux ouverts.

Trois parades, selon ce que l'équipe accepte de payer :

- **`schedule`** dans le workflow — la réconciliation périodique du §6.4 ; elle borne la fenêtre à l'intervalle choisi. GitHub désactive les workflows planifiés d'un dépôt **public** resté 60 jours sans activité : sur un dépôt vivant, le cas ne se présente pas ;
- ***Require conversation resolution before merging*** — natif, gratuit, ferme complètement la fenêtre, au prix d'exiger la résolution de **tous** les fils et non des seuls bloquants ;
- **ne rien faire**, si le modèle de menace de l'équipe ne retient pas « un relecteur dé-résout un fil sans rien écrire, et quelqu'un merge avant le déclencheur suivant ».

**2. Sur une PR issue d'un fork, le `GITHUB_TOKEN` est en lecture seule — y compris sur les événements de revue.** La restriction est connue pour `pull_request` ; la documentation la pose **aussi** pour `pull_request_review` et `pull_request_review_comment`, dont le `GITHUB_REF` désigne la branche de fusion de la PR. Une exécution déclenchée par le commentaire de revue d'un contributeur externe ne peut donc **pas** publier de check run.

Deux déclencheurs y échappent, parce qu'ils s'exécutent dans le contexte de la branche par défaut du dépôt de base : `pull_request_target` et `issue_comment`. Ils couvrent le cycle de vie de la PR et les commentaires de conversation — **pas** les commentaires de revue ni les revues soumises, c'est-à-dire pas ceux qui ouvrent les fils bloquants.

Ces PR ne restent donc pas sans statut — `pull_request_target` en publie un à l'ouverture, à chaque push et à chaque changement d'étiquette ou d'état brouillon, ce qui suffit à ne jamais bloquer une PR de fork par un check absent. Ce qui manque est le **rafraîchissement** : un commentaire de revue bloquant écrit après le dernier push ne rougit pas le check, et c'est le sens de divergence dangereux.

Un dépôt qui accepte des PR de forks **et** veut le blocage sur ces PR ajoute donc le **workflow compagnon** : un premier workflow, déclenché par les événements de revue, ne fait rien d'autre que s'exécuter ; un second, déclenché par `workflow_run`, s'exécute dans le contexte du dépôt de base avec un jeton en écriture et réévalue les PR ouvertes. C'est le contournement que GitHub documente pour ce cas, et il reste sans serveur. Un dépôt dont toutes les PR viennent de branches internes — le cas d'entreprise courant — n'en a pas besoin.

> **Pourquoi `pull_request_target` est sans danger ici, alors qu'il est un piège ailleurs.** Ce déclencheur est déconseillé dès qu'un workflow **construit ou exécute** le code proposé, puisqu'il lui donne un jeton en écriture et les secrets du dépôt. Le vérificateur ne fait **jamais** de `checkout` de la PR et n'exécute rien qui en provienne : il ne lit que des données d'API. La condition est structurelle et doit le rester.

**3. Le bouton de rejeu d'un check run créé par une Action ne rejoue rien.** GitHub émet bien `check_run` (`rerequested`) comme déclencheur de workflow, mais **pas lorsque la *check suite* a été créée par GitHub Actions** — c'est sa garde anti-récursion. Le déclenchement manuel du §6.4 passe donc par le **rejeu du workflow**, natif dans l'onglet des exécutions et sur la ligne du job dans la PR, et non par une action portée par le check `conventional-comments`. C'est un clic de plus, au même endroit.

#### A.8.3 Publication

- Le *check run* est nommé `conventional-comments` et calculé selon les deux critères du §6.2, en s'appuyant sur GraphQL pour l'état des fils (§A.6).
- **Restitution lisible par l'extension** (§6.3.1) : la ligne `cc/1` est publiée dans le **titre** du *check run*, que GitHub rend sur la page de la PR. Le **corps** Markdown porte la sortie humaine du §6.3.1 — fils, auteurs, liens permanents — et n'a pas à être reparsé : l'extension tient ses ancres du DOM de la page.
- **Où va quoi.** La ligne machine `cc/1` occupe l'`output.title` — c'est ce que GitHub rend sur la page de la PR, donc le seul emplacement que `readPublishedResult()` puisse lire. Le **résumé humain** (`ComplianceResult.headline`) ouvre le corps Markdown, suivi du détail par fil et par diagnostic. Les deux ne se disputent pas le même champ.
- **Correspondance des états** (§9.2.1) : `state: 'success'` → conclusion `success`, `'failure'` → `failure`, `'neutral'` → `neutral`. GitHub compte `neutral` parmi les conclusions qui **satisfont** une vérification obligatoire, au même titre que `success` et `skipped`.
- **Permissions du jeton** : `checks: write` pour publier, `pull-requests: read` pour lire fils, commentaires et étiquettes, `contents: read` pour le fichier de configuration. `pull-requests: write` s'ajoute **uniquement** si le dépôt emploie le chemin de repli d'exemption du §6.3.2 — sur GitHub, la provenance des étiquettes étant lisible, il ne l'emploie pas.
- Ce check est déclaré **required** dans la protection de branche → le bouton *Merge* est désactivé nativement.
- Activer en complément *Require conversation resolution before merging* si l'équipe souhaite l'exigence sur **tous** les fils, pas seulement les bloquants — c'est aussi la parade complète à la limite 1 ci-dessus.

#### A.8.4 GitHub Enterprise Server

GHES exécute Actions et expose les mêmes API. La **version minimale** qui porte l'ensemble des déclencheurs employés ici — `pull_request_target` et le contexte de jeton associé en particulier — **est à établir sur l'instance visée avant activation**, et n'est pas affirmée par ce document : elle dépend de la version installée, que rien ici ne connaît. Le profil B (§6.4.1) reste disponible comme repli si une instance ne porte pas ces déclencheurs.
...
### B.7 Mise en œuvre du vérificateur (composant B)
