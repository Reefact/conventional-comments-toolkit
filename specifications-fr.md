...
- il commence par une **commande adressée à un outil**. Deux formes, reconnues chacune sur le **premier jeton du corps**, lequel doit être suivi d'une espace ou de la fin de ligne :
  - **commande slash** — le jeton a la forme `/[A-Za-z][A-Za-z0-9_-]*` (`/azp`, `/retest`, `/lgtm`, `/deploy`). La reconnaissance est **générique** : aucune liste de commandes n'est tenue, seule la forme compte. Elle n'est active que sur les plateformes où ces commandes existent, ce que dit `PlatformProfile.slashCommands` (§9.2) ;
  - **interpellation d'un compte de robot** — le jeton a la forme `@handle`, et le handle figure dans la liste **fermée** `PlatformProfile.commandPrefixes` (§9.2), donnée en annexe (`@codex review`, `@dependabot rebase`). `@codex review` est une commande ; `@codexplique pourquoi` n'en est pas une, la reconnaissance portant sur le jeton entier ;
...
**Pourquoi `/` est générique et `@` ne l'est pas.** L'asymétrie n'est pas une prudence inégale, elle vient de ce que les deux caractères désignent.

Le slash **n'ouvre pas une remarque de revue**. Aucune phrase de relecteur ne commence par lui, et la contrainte de forme du jeton écarte le seul faux positif crédible — le chemin : `/etc/hosts n'est pas le bon endroit` et `/api/v1/users devrait être versionné` portent un second `/` dans le jeton et ne sont donc pas exemptés. Reste `/tmp est plein`, qu'on assume. En échange, aucune liste n'est à tenir : les commandes Prow, les *comment triggers* d'Azure Pipelines, les `/deploy` maison et toute commande future sont couverts sans que rien ne soit édité. Le contournement — écrire `/x en fait ce nom est ambigu` — est réel, et c'est **exactement** celui du bloc délimité ci-dessous : il n'atteint que le critère 1, l'hygiène de forme, dont la sévérité par défaut est un avertissement. Un commentaire exempté n'ouvre aucun fil bloquant, donc n'exempte personne de traiter quoi que ce soit, et le §2 pose déjà que le composant A est contournable par construction. Un dépôt reste par ailleurs libre d'écrire `^/…` lui-même dans `allowlistPatterns` (§8.2) — comme il l'est pour `^@\w+` ou `.*`, sous les mêmes bornes purement techniques. Ce serait redondant, pas interdit : c'est exactement ce que la reconnaissance générique du produit couvre déjà, sans qu'aucun dépôt n'ait à l'écrire ni à l'entretenir.

L'arobase, elle, **ouvre couramment une remarque de revue** : interpeller quelqu'un est l'une des façons les plus naturelles de commencer un commentaire. `@alice peux-tu regarder ça ?` en est une, et un `^@\w+` générique l'exempterait. Le risque n'est plus le contournement — qui suppose une intention — mais le **faux négatif silencieux** : une classe entière de vrais commentaires sortirait du périmètre sans que personne ne l'ait voulu ni ne s'en aperçoive, précisément sur la zone où la convention sert le plus. C'est une tout autre nature de défaut, et elle ne s'assume pas. Les handles de robot sont donc **nommés un par un** en annexe, portés par l'adaptateur, hors d'atteinte de la configuration d'un dépôt. Un robot **interne**, absent de cette liste, relève de `allowlistPatterns` (§8.2) sous la forme d'un motif ancré et exact — `^@notre-bot\b` —, sous la responsabilité du dépôt qui l'ajoute.

**Résoudre le handle plutôt que le nommer n'est pas une option.** L'idée se présente d'elle-même : exempter `@handle` lorsque le compte interpellé *est* un robot, ce que les deux plateformes savent dire. Elle bute sur le §2 et sur `CA-06`. La validation s'exécute **à la frappe** dans le navigateur et doit rendre le même verdict que le serveur, au caractère près ; une résolution de compte la rendrait asynchrone, dépendante du réseau et de droits de lecture, et ferait dépendre le verdict d'un état extérieur que les deux composants ne voient pas au même instant. La liste fermée est ce qui permet à `validate()` de rester une fonction pure de son entrée.
...
Le champ `exemptUsers` de l'exemple est un espace réservé : les identités de comptes de service diffèrent d'une plateforme à l'autre et sont données dans les annexes. Aucun motif exemptant un préfixe de commande — `^/…`, et moins encore `^@\w+` — ne figure dans les valeurs par défaut. Les commandes légitimes relèvent de l'exemption structurelle du §4.2, portée par l'adaptateur, pas d'un motif configurable, et la distinction n'est pas une redite : ce que la plateforme accorde structurellement a une forme arrêtée qu'un dépôt ne peut ni élargir ni assouplir, là où un motif par défaut serait le point de départ de toutes les variantes que chaque dépôt jugerait commodes. Un dépôt reste en revanche libre d'écrire `^@\w+` dans `allowlistPatterns` — comme il reste libre d'y écrire `.*`, qui exempterait tout : ces motifs sont fournis par le dépôt (paragraphe suivant), et les seules bornes qu'ils connaissent sont techniques — taille, nombre, absence de quantificateur imbriqué —, jamais sémantiques. Un tel motif y produirait le même effet que dans les valeurs par défaut, exempter l'interpellation d'une personne, mais ce serait alors un choix du dépôt sur son propre périmètre, opposable à lui seul, et non une case que le produit aurait laissée ouverte. Un robot **interne** se déclare de la même façon, sous une forme ancrée et exacte (`^@notre-bot\b`) plutôt que générique.
...
interface EvaluationInput {
  ...
  platform: PlatformProfile;           // §3.5.1 étages −1 et 0 ; sans lui, aucun bloc de suggestion
                                       // ni commande d'outil n'est reconnu côté serveur, et `CA-06` échoue
  threads: ThreadInfo[];
...
interface PlatformProfile {
  ...
  slashCommands: boolean;              // §4.2 — la plateforme reconnaît-elle les commandes slash ? Si oui,
                                       // tout premier jeton de la forme `/[A-Za-z][A-Za-z0-9_-]*` exempte,
                                       // sans qu'aucune commande soit énumérée. Un booléen et non une liste :
                                       // la forme suffit à distinguer une commande d'une phrase de revue
  commandPrefixes: string[];           // §4.2 — handles des comptes de robot dont l'interpellation exempte
                                       // (`@codex`, `@dependabot`). Liste **fermée** — ici la forme ne
                                       // suffit pas, `@alice peux-tu regarder ça ?` étant une remarque de
                                       // revue —, portée par l'adaptateur, donc hors d'atteinte de la
                                       // configuration d'un dépôt (§4.2, §8.2)
}
...
- **Commandes adressées à un outil** (§4.2). GitHub en connaît les deux formes, et elles sont portées différemment (§9.2) :
  - **Préfixe slash** — `slashCommands: true`. Rien n'est énuméré : la forme du jeton suffit, et couvre du même coup `/rebase`, les *comment triggers* d'Azure Pipelines `/azp` (fonctionnalité **GitHub uniquement**, absente d'Azure Repos), les commandes Prow répandues dans l'écosystème CNCF (`/lgtm`, `/approve`, `/hold`, `/retest`, `/override`…), les `/deploy` d'une Action maison et toute commande qu'une App installée plus tard reconnaîtrait. Une liste nominative serait ici un entretien permanent pour aucune garantie supplémentaire (§4.2).
  - **Interpellation d'un compte de robot** — `commandPrefixes`, nommés un par un : `@dependabot` (`rebase`, `recreate`, `merge`, `squash and merge`, `cancel merge`, `reopen`, `close`, `ignore this major version`…), `@copilot`, `@coderabbitai` (`review`, `full review`, `resolve`, `pause`, `resume`, `ignore`, `configuration`, `help`), `@codex`, `@claude`, `@mergifyio` (`refresh`, `update`, `rebase`, `backport <branche>`, `queue`), `@renovate`, `@rustbot`, `@bors`. C'est la forme la plus employée, et celle dont l'absence coûterait le plus cher : sans elle, `@codex review` tombe en `E-NO-LABEL` et le mode `enforce` **empêche de relancer un relecteur automatique** — la zone de conversation générale, où ces commandes s'écrivent, est validée en format (§4.1).

  Les identités de robot citées ici le sont comme **valeurs**, sujettes à l'évolution de l'écosystème : la liste s'actualise avec l'adaptateur, jamais avec la configuration d'un dépôt (§4.2). C'est le seul entretien que ce mécanisme demande, et il est borné à cette liste-là. `bors r+` et `r? @alice`, qui ne portent **ni slash ni mention**, n'y figurent pas et ne peuvent pas y figurer : `bors` seul exempterait « bors est cassé depuis hier ». Un dépôt qui les emploie les déclare en `allowlistPatterns` sous forme ancrée et exacte — `^bors\s+(r\+|r=|try|retry|ping|delegate)` —, ce que les bornes du §8.2 autorisent.
...
- **Aucune commande native, ni slash ni mention.** Les *comment triggers* du type `/azp run` sont une fonctionnalité d'Azure Pipelines réservée aux dépôts **GitHub** ; Azure Repos passe par les policies de branche. Aucun écosystème de robots pilotés au commentaire ne s'y est établi non plus : la revue de code assistée que la plateforme propose se déclenche par son interface, pas par une interpellation dans un fil. `slashCommands` vaut donc **`false`** et `commandPrefixes` est **vide** (§9.2) : l'exemption du §4.2 n'a ici aucune illustration native. Le slash n'y est pas reconnu génériquement comme sur GitHub, et c'est délibéré — sur une plateforme où aucune commande n'existe, la reconnaissance générique n'exempterait que des contournements. Une organisation qui installe un outil tiers piloté au commentaire n'a pas de bascule à sa disposition — `slashCommands` et `commandPrefixes` sont portés par l'adaptateur, pas par la configuration (§9.2) — mais dispose du même repli que pour un robot **interne** côté GitHub (§A.7) : un motif ancré et exact dans `allowlistPatterns` (§8.2), à la charge du dépôt qui l'ajoute.
