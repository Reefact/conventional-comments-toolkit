...
- `CA-40` **Commande adressée à un outil** (§4.2, §8.2), **à défaut d'une autre exemption du même §4.2** — un motif de `allowlistPatterns`, un auteur exempté ou un message de plateforme priment, quel que soit le contenu de `toolCommands`. `toolCommands` étant **vide par défaut**, `@codex review` et `/rebase` restent des remarques de revue et produisent `E-NO-LABEL` tant qu'ils sont absents de la configuration **appliquée à la PR évaluée** au sens du §8 — jamais d'un seul de ses niveaux pris isolément : les trois niveaux du §8.1.2 y concourent, les bornes du §8.1.1 peuvent en écarter une entrée, et le mélange épinglé du §8.1.3 peut en retenir une que le dépôt a retirée depuis. Le sentinel `/*` exempte alors toute commande slash — `/azp run`, `/lgtm` — sans qu'aucune ne soit énumérée, tandis qu'un chemin comme `/etc/hosts n'est pas le bon endroit` porte un second `/` dans son premier jeton et reste validé. Un handle déclaré exempte la commande sans égard à la casse — `@Codex review` comme `@codex review` —, un handle absent de la liste ne l'exempte pas, et la reconnaissance porte sur le **jeton entier** : `@codexplique pourquoi` n'est pas une commande. `@alice peux-tu regarder ça ?` n'est exempté que si `@alice` figure **explicitement** dans la liste : aucun sentinel générique n'existe pour les mentions, si bien qu'aucune configuration ne peut les exempter en bloc — l'asymétrie avec `/*` que le §4.2 justifie.
