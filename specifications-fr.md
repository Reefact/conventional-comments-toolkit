...
**Comment lire ces critères.** Sauf mention contraire, chacun s'entend sur un commentaire que le §3.5.1 conduit **jusqu'à l'étage où le critère se place** — zone validée par `scope`, aucune exemption du §4.2 applicable — et sur une PR **dans le périmètre d'activation** (§6.2.3). Ces conditions sont celles de toute évaluation : les répéter quarante fois n'apprendrait rien, mais les taire rendrait chaque critère faux pour une configuration pourtant supportée. Sous `scope.validateReplies: false`, un `Attention : le build casse` écrit en réponse de fil ne produit **aucun** diagnostic, et il en va de même sous un motif de `allowlistPatterns`, un auteur exempté ou un message généré par la plateforme — le résultat qu'un critère énonce ne vaut donc jamais qu'à son propre étage.
