...
```text
issue (blocking): Platform CSS tokens leak into the shared stylesheet

The shared stylesheet is also loaded on Azure DevOps.
```
```text
done: Removed the platform-specific token

The guard now detects tokens declared by each platform stylesheet.
```

```text
suggestion: Extract this mapping into the adapter.
```
```text
disagree: The mapping is intentionally platform-independent

Moving it to the adapter would duplicate the same rule between platforms.
```

```text
nitpick: This variable name reads oddly.
```
```text
ack: Fair, I'll keep it in mind for the next module.
```
...
| `lgtm`, `r+`, `+1` | Ces jetons s'adressent à une **machine**, et le §4.2 les **exempte** par construction — `allowlistPatterns` prend `^LGTM$` pour exemple. En faire un label rendrait en badge une chaîne que le même document est bâti pour écarter. Les labels de réponse s'adressent à une **personne** et sont validés. |
...
- **Les labels de réponse (§3.2) sont des boutons de label comme les autres**, dans la **même rangée**, où ils se replient comme les autres. Ni troisième rangée, ni séparateur, ni section « Réponses ». La règle « deux rangées empilées, jamais une seule » se justifie par la **nature du geste** — poser un label contre appliquer une décoration —, non par leur nombre : poser `done:` et poser `issue:` sont le même geste, insérer ou remplacer un préfixe. Une séparation visuelle dirait le contraire, et c'est exactement le contresens à éviter, celui qui ferait ensuite restreindre ces labels aux réponses. La famille se distingue par le libellé de l'infobulle et par la documentation, jamais par la mise en page.
  ...
  **La géométrie est à re-mesurer, pas à supposer.** Un dépôt qui active le vocabulaire passe de dix à **treize** boutons — le pire cas que le produit assume et teste déjà. Il en atteint **seize** s'il active aussi `typo`, `polish` et `quibble`, ce qu'aucune mesure ne couvre. `check:composer-layout` et `check:subject-line` doivent donc être rejoués aux deux paliers, et le pire cas documenté relevé en conséquence. Si seize brise la mise en page, la réponse honnête est qu'un dépôt ne peut pas raisonnablement activer les deux familles optionnelles à la fois — pas d'ajouter une rangée.
...
**Clés soumises au plancher** (« plancher-ables ») : `mode` (via `minimumMode`), `formatSeverity`, `severities`, `labels[].enabled` et `labels[].blockingByDefault` (via `minimum`), `activation.activatedAt`, `rules.minDecisionSubjectLength`, `exemptUsers`, `allowlistPatterns`, `toolCommands`, `resolverOverrideGroup` et `configCacheTtlSeconds`. Ce sont celles qui, laissées libres, permettraient soit de se soustraire à la contrainte, soit de faire diverger les deux composants. Les clés purement cosmétiques ou ergonomiques — libellés, icônes, langue, style de badge, seuils de longueur du sujet — restent librement surchargeables.
...
| `labels` | `minimum` : ensemble d'`id` dont ni `enabled` ni `blockingByDefault` ne peuvent passer à `false` en dessous du plancher. Le plancher **force toujours `enabled`** ; il ne **relève jamais** `blockingByDefault` sur un label dont l'`alwaysNonBlocking` effectif vaut `true` — voir la seconde justification ci-dessous. C'est aussi la clé dont l'absence viderait le critère 2. |
...
**Aucun ombrage entre labels et alias.** Dans la configuration résolue, un identifiant de label ne doit être ni l'`id` ni un alias d'un autre label ; une entrée qui violerait cette règle est **ignorée et signalée** (`config-warning`, §9.2.1) plutôt que résolue en silence vers l'autre. Sans elle, c'est l'**ordre** de la liste qui déciderait du verdict — la résolution compare, label par label dans l'ordre, l'`id` puis les alias, si bien qu'un label déclarant `done` en alias et placé avant le label `done` capterait toutes les saisies. La règle est générale et vaut depuis toujours ; ce sont les labels de réponse qui la rendent atteignable, en réservant trois identifiants qu'une équipe a pu employer avant eux.

**Réserver un identifiant a un coût, et il se documente plutôt qu'il ne se découvre.** Une équipe qui possédait déjà un label `done` verra son entrée **fusionner** avec celle que le produit livre (§8.1.4, fusion par `id`) et hériter de `alwaysNonBlocking: true` : son `done (blocking):` jusque-là valide se met à produire `E-CONFLICT`. Le sens de l'écart est sûr — le label devient moins bloquant, jamais plus — et la réparation tient en une ligne dans son propre fichier, `"alwaysNonBlocking": false`, une déclaration explicite l'emportant toujours. Le précédent est exact et n'avait, lui, jamais été écrit : livrer `typo`, `polish` et `quibble` avait réservé trois noms de la même façon. Les trois identifiants réservés ici sont `ack`, `done` et `disagree` ; ils appellent une entrée de changelog et une note de migration, pas une découverte en revue.
...
Reste l'empreinte, et c'est là qu'il faut être précis plutôt que rassurant. Un dépôt qui écrit la forme **minimale** — `{"id": "done", "enabled": true}` — laisse le `core/` antérieur inventer les valeurs manquantes par défaut : mesuré, `27de281a` contre `934e619d`. Le désaccord est **visible** et va dans le sens sûr — la règle 2 du §8.1.3 désarme le blocage d'envoi le temps du décalage —, et c'est un état parfaitement acceptable.

Un dépôt qui veut fermer la fenêtre doit déclarer la définition **complète, alias compris** — et le piège est là : les quatre champs `enabled`, `blockingByDefault`, `alwaysNonBlocking` et `aliases`. Une déclaration à trois champs *paraît* complète et ne l'est pas ; mesuré, elle donne `32331a37` contre `934e619d`, quand la déclaration avec alias donne `934e619d` des deux côtés — zéro désaccord. Recopier un jeu d'alias interne pour éviter une fenêtre transitoire reste un choix fragile, qui couple le fichier d'un dépôt à un détail de `core/` ; la forme minimale est celle qu'on recommande, et la fenêtre le prix connu.
...
- `CA-44` **Le plancher ne rend jamais bloquant un label toujours non bloquant.** Un plancher `{"labels": {"minimum": ["ack", "done", "disagree"]}}` force `enabled: true` sur les trois entrées et laisse leur `blockingByDefault` à `false` : un `done:` nu **n'ouvre pas** de fil bloquant, et la paire incohérente `alwaysNonBlocking: true` + `blockingByDefault: true` n'est jamais produite (§8.1.1). Contre-épreuve sur le défaut antérieur : la même propriété vaut pour `decision`, dont le §6.1.1 affirme qu'une décision ne peut pas elle-même bloquer une PR.
