...
**Le coût de ce retrait doit être dit.** Durcir la configuration d'un dépôt peut désormais faire passer au rouge une PR ouverte, sur un commentaire que son auteur avait écrit conforme. Trois choses le bornent, et c'est ce qui rend le retrait acceptable : le critère 1 n'échoue pas par défaut (`formatSeverity: warn`, §6.2.1), donc un durcissement de forme ne bloque personne ; `activation.activatedAt` (§6.2.3) tient hors périmètre les PR antérieures à la bascule ; et le sens qui compte — un fil bloquant non résolu — ne dépend d'aucun durcissement, seulement de ce que la PR contient. Une organisation qui veut malgré tout protéger les PR en vol dispose du levier ordinaire : durcir en `warn`, mesurer, puis passer en `enforce`.
...
**`resolveConfig()` ne prend que ce qu'on vient de lire.** Ni configuration épinglée, ni drapeau « dépôt déjà évalué » : la première demandait une mémoire par PR (§8.1.3), le second une mémoire par dépôt. Le fichier de dépôt absent est donc toujours traité comme la **disparition** du §8.1.5 et non comme un dépôt jamais activé — ce qui est le bon verdict, puisque le vérificateur ne s'exécute que là où quelqu'un l'a installé : sa présence *est* l'activation.
...
interface ServerPlatformAdapter {
  ...
  currentPr(): Promise<PrRef>;                       // §6.4 — la PR que l'exécution courante évalue,
                                       ...
                                       // souscription à des webhooks, ni signature à vérifier, ni
                                       // séquence à attribuer : le vérificateur est réveillé par
                                       // l'intégration continue, pas par un message qu'il reçoit
