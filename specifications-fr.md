...
- **Sur le chemin de repli, l'exemption persistée et son étiquette forment un seul état.** L'une sans l'autre ment : une exemption sans étiquette est une PR verte que rien n'explique sur la page, une étiquette sans exemption est un marqueur qui n'accorde rien. Deux règles normatives tiennent cet invariant :

  1. **L'octroi n'est effectif qu'une fois l'étiquette posée.** Le point d'entrée d'administration écrit l'exemption **en attente**, appelle `addLabel()`, puis la **confirme** ; `evaluate()` ne reçoit dans `ctx.exemption` (§9.2.2) qu'une exemption **confirmée**. Si `addLabel()` échoue — indisponibilité, quota, droit momentanément refusé —, l'enregistrement en attente est supprimé et **l'opération est refusée** : le point d'entrée ne répond « exemption accordée » que lorsque les deux morceaux existent. L'état intermédiaire n'est pas un excès de prudence, il est ce qui rend la règle applicable : sans lui, une interruption entre les deux écritures laisse un enregistrement que rien ne distingue d'un octroi réussi, et la PR passe au vert sans que `cc-override` apparaisse nulle part. Si la suppression échoue à son tour, rien ne fuit — une exemption en attente n'est jamais admise, et la première évaluation qui en rencontre une l'élimine.
  2. **Une exemption confirmée dont l'étiquette a disparu fait restaurer l'étiquette**, elle ne fait pas tomber l'exemption. Sur ce chemin l'étiquette est **définie comme un marqueur produit par le composant B**, jamais comme le titre lui-même : la retirer à la main ne peut donc pas plus modifier une décision d'autorisation que la poser n'en accorde une. `evaluate()` le demande par `actions.addLabel` et le signale (`exemption-label-restored`, §9.2.1) ; l'orchestrateur l'exécute à l'étape 16 du §6.4. **La conséquence doit être dite : sur ce chemin, retirer l'étiquette ne révoque rien.** La révocation passe par le point d'entrée qui a accordé (§6.2.4) ou par la remise à zéro automatique ci-dessous. C'est la contrepartie exacte de la règle : là où la provenance est exposée, l'étiquette *est* le titre et la retirer révoque ; ici elle ne l'est pas, et il faut un geste qui l'est.
...
16. Une fois la publication **réussie**, persister le résultat publié et poser le
    ...
    supprime aussi l'exemption active persistée, ou addLabel, qui restaure
    l'étiquette d'une exemption confirmée — jamais les deux                         §6.4, §6.3.2
```
...
- l'**exemption active de chaque PR ouverte** — `{by, at}` **et l'état de son étiquette**, en attente tant que la pose n'a pas réussi, confirmée ensuite —, **distincte du journal ci-dessus** : celui-ci est un historique, qui garde aussi les exemptions révoquées ; celle-là dit ce qui vaut maintenant. Elle n'existe que sur le **chemin de repli** du §6.3.2, là où la plateforme n'expose pas la provenance d'une étiquette : le point d'entrée d'administration vérifie l'habilitation, persiste l'exemption en attente, pose l'étiquette (`addLabel()`, §9.2.4), puis la confirme. **Seule une exemption confirmée est admise** ; une exemption en attente rencontrée par une évaluation est supprimée, jamais retenue. Sans cet objet, le tour suivant ne verrait qu'une étiquette sans auteur ni date et ne reconnaîtrait pas l'exemption qu'il avait lui-même accordée ; sans l'état de son étiquette, une interruption entre les deux écritures produirait une PR verte sans marqueur visible. La remise à zéro du §6.3.2 la supprime en même temps qu'elle retire l'étiquette, et le point d'entrée du §6.2.4 fait de même à la révocation. Là où la provenance est exposée, cet objet reste vide : l'exemption se relit de la PR à chaque tour ;
...
| `exemptUsers`, `allowlistPatterns`, `toolCommands` | retrait d'une entrée | ajout d'une entrée |
...
**Deux clés sur la même zone, de polarités opposées.** `scope.validateReplies` et l'activation d'un label touchent toutes deux les réponses de fil, et se classent en sens **contraires** : le passage de `validateReplies` à `true` est **restrictif** — il ajoute des diagnostics —, tandis qu'un `enabled: false → true` est **élargissant** — il en retire. Ni l'un n'implique l'autre (§4.1), et le classement de l'un ne se recopie pas sur l'autre par analogie : il en sortirait à l'envers.

**À quelle granularité.** Le classement porte sur **une clé à la fois**, et sur **une entrée à la fois** pour les clés de liste (`labels`, `decorations.known`, `exemptUsers`, `allowlistPatterns`, `toolCommands`). Un même commit peut retirer un label et en ajouter un autre : il n'est ni restrictif ni élargissant dans son ensemble, et vouloir le classer d'un bloc n'a pas de réponse. Entrée par entrée, si : le retrait est épinglé, l'ajout s'applique en direct, dans le même commit et sans qu'aucune règle particulière soit nécessaire.
...
**Ce qu'il advient si un dépôt adopte cette clé avant que son serveur ne la connaisse**, et pourquoi cela ne demande ni changement de `version` ni de `coreMinVersion`. Le cas est réel : l'extension se met à jour depuis un store, le serveur au rythme de l'organisation, et la règle 5 du §8.1.3 pose que l'écart est le cas **normal**. Un serveur d'une version antérieure lit alors `toolCommands` comme une clé inconnue, l'ignore et le signale (§8.1.5) ; l'extension, elle, l'applique. La commande est exemptée à la saisie et rejetée par le check.
...
interface ComplianceResult {
  ...
  actions: { removeLabel?: string;     // §6.3.2 — ce que l'orchestrateur doit **faire** en plus de publier.
             addLabel?: string };      // `evaluate()` est pure : elle demande le retrait ou la
                                       // restauration de l'étiquette, elle ne les exécute pas (§9.2.4).
                                       // Le retrait emporte la suppression de l'exemption active
                                       // persistée (§6.4) : une seule demande, parce que les deux ne se
                                       // dissocient jamais. La restauration ne vaut que sur le chemin de
                                       // repli, pour une exemption **confirmée** dont l'étiquette a
                                       // disparu. **Jamais les deux à la fois**
...
interface EvaluationInput {
  ...
  platform: PlatformProfile;           // §3.5.1 étage 0 ; sans lui, aucun bloc de suggestion n'est
                                       // reconnu côté serveur, et `CA-06` échoue
  threads: ThreadInfo[];
...
interface ServerPlatformAdapter {
  ...
  removeLabel(pr: PrRef, name: string): Promise<void>;   // §6.3.2 — remise à zéro de l'exemption. Avec
                                       ...
                                       // du statut. **Les deux sont idempotentes** : poser une étiquette
                                       // déjà présente ou en retirer une absente est sans effet, jamais
                                       // une erreur. La règle de restauration du §6.3.2 rend ces deux
                                       // états atteignables en marche normale — une remise à zéro peut
...
- `CA-39` **Cohérence de l'exemption sur le chemin de repli** (§6.3.2). Une pose d'étiquette qui échoue ne laisse **aucune exemption active** et l'appel d'administration répond en échec. Une exemption confirmée dont quelqu'un retire l'étiquette à la main la voit **restaurée** au tour suivant, avec un `notice`, et le statut reste vert : sur ce chemin, retirer l'étiquette ne révoque pas — la révocation passe par le point d'entrée du §6.2.4, qui supprime l'exemption **et** l'étiquette.
