...
**Comment lire ces critères.** Ceux dont le résultat énoncé est un **verdict porté sur un commentaire** — un diagnostic, une conformité, un blocage de saisie — s'entendent, sauf mention contraire, sur un commentaire que le §3.5.1 conduit **jusqu'à l'étage où le critère se place** : zone validée par `scope`, aucune exemption du §4.2 applicable, PR **dans le périmètre d'activation** (§6.2.3). Ces conditions sont celles de toute validation : les répéter n'apprendrait rien, mais les taire rendrait ces critères faux pour une configuration pourtant supportée. Sous `scope.validateReplies: false`, un `Attention : le build casse` écrit en réponse de fil ne produit **aucun** diagnostic, et il en va de même sous un motif de `allowlistPatterns`, un auteur exempté ou un message généré par la plateforme — le verdict qu'un tel critère énonce ne vaut donc jamais qu'à son propre étage.
...
Un critère qui dépend de l'une de ces conditions **autrement que par défaut** le dit explicitement : ainsi `CA-07`, qui repose sur l'exclusion des réponses, `CA-15` et `CA-01`, qui portent sur le périmètre, ou `CA-08`, `CA-20` et `CA-40`, qui portent sur les exemptions elles-mêmes.
