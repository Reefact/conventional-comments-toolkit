...
12. Si le résultat porte E-UNKNOWN-LABEL, E-UNKNOWN-DECORATION, ou E-NO-LABEL sur un
    corps dont le premier jeton commence par `/` ou `@` : seconde passe
...
Avant d'émettre un rejet qui dépend de la configuration — `E-UNKNOWN-LABEL`, `E-UNKNOWN-DECORATION`, ainsi que le cas de `toolCommands` décrit juste après — le composant B rafraîchit **les deux niveaux** de configuration en contournant leur cache : un label peut avoir été ajouté au fichier du dépôt aussi bien qu'au document d'organisation.

**`toolCommands` ouvre un troisième déclencheur, et il demande un discriminant.** Ajouter une entrée y est élargissant (tableau ci-dessus), donc appliqué en direct ; mais le symptôme d'une entrée manquante n'est pas un code propre — c'est `E-NO-LABEL`, le diagnostic le plus courant de tous. Déclencher une seconde passe sur chaque `E-NO-LABEL` contournerait le cache sur la quasi-totalité des commentaires non conformes, et ruinerait le « seulement quand il le faut » que ce paragraphe promet. Le déclencheur est donc **qualifié** : un `E-NO-LABEL` sur un commentaire dont le **premier jeton a la forme d'une commande** — il commence par `/` ou `@` — sans qu'aucune entrée de `toolCommands` ne l'ait reconnu. Une remarque ordinaire sans label (« il faudrait renommer ça ») ne commence ni par l'un ni par l'autre et ne déclenche rien ; seul un corps qui *ressemble* à une commande sans en être une paie la seconde passe.

Sans ce troisième déclencheur, la ligne `toolCommands` du tableau serait vraie sur le papier et fausse en pratique pendant toute la durée du cache : un dépôt qui vient d'ajouter `@codex` verrait son extension exempter la commande — elle résout la configuration pour son propre compte — pendant que le serveur la rejette encore. C'est exactement l'asymétrie « extension en avance sur le serveur » que la fin de cette section désigne comme le sens de divergence qui dégrade une PR.

Concrètement, l'évaluation se fait **en deux passes, et seulement quand il le faut** : une première passe avec la configuration en cache ; si elle produit l'un des trois déclencheurs ci-dessus, une seconde après `fetchConfigFile(pr, { bypassCache: true })` **et** `fetchOrgConfig(url, { bypassCache: true })` (§9.2.4). Les **deux** niveaux, comme le dit le paragraphe précédent et comme l'exige l'étape 12 du §6.4 : le niveau du dépôt n'est pas le moins important des deux, c'est au contraire celui où `toolCommands` se règle le plus souvent. Seul le verdict de la seconde passe est publié. Les fonctions de `core/` étant pures, c'est l'orchestrateur serveur qui porte cette boucle — elle ne peut pas vivre dans `validate()`.

**Un contournement stérile ne se répète pas.** Un contournement déclenché par une configuration réellement en retard se tarit de lui-même : la seconde passe trouve la nouvelle valeur, le rejet disparaît, le déclencheur avec lui. Mais un rejet qui ne vient **pas** d'un cache en retard, lui, persiste — `@alice peux-tu regarder ça ?` a la forme d'une commande sans en être une, et le restera tant que le commentaire est là. Sans mémoire, chaque webhook et chaque réconciliation relanceraient la double lecture : le TTL serait annulé pour ce dépôt aussi longtemps que ce commentaire y figure, et d'autant plus lourdement que la réconciliation est fréquente — 60 s sur Azure DevOps (§B.7).

La règle est donc : lorsqu'un contournement rend **exactement** la configuration déjà en cache, il est enregistré comme **stérile**, et n'est pas retenté avant l'expiration ordinaire du TTL. La seconde évaluation est alors inutile et n'a pas lieu — son verdict serait identique au caractère près. L'étranglement ne porte que sur les contournements stériles : un contournement fructueux n'est **jamais** étranglé, et n'a pas à l'être puisqu'il ne se reproduit pas. Une lecture `unreachable` n'est jamais tenue pour « la même » qu'une autre : ce n'est pas un état stable, et la compter comme telle étranglerait les contournements sur une panne passagère.

**L'étranglement porte sur le couple (dépôt, rejets déclencheurs), jamais sur le dépôt seul.** Un contournement stérile n'établit rien sur *ces* rejets-là ; il n'établit rien sur les suivants. Étrangler le dépôt entier ferait qu'un `@alice peux-tu regarder ça ?` laissé ouvert empêcherait, pendant tout le TTL, le rafraîchissement dû à un `@codex` ajouté après lui ou à un label nouvellement déclaré — c'est-à-dire précisément les modifications élargissantes que la règle 1 exige d'appliquer en direct. On aurait remplacé une amplification par une suppression.

La signature des déclencheurs retient le **jeton**, et pas seulement le code du diagnostic : `@alice …` et `@codex …` produisent l'un comme l'autre un `E-NO-LABEL`, et un contournement resté vain pour le premier n'apprend rien sur le second. Elle change donc dès qu'un rejet apparaît, disparaît ou change de jeton, et le contournement est alors de nouveau tenté.
