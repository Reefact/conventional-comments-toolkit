...
**Comment lire ces critères.** Ceux dont le résultat énoncé est un **verdict porté sur un commentaire** — un diagnostic, une conformité, un blocage de saisie — s'entendent, sauf mention contraire, sur un commentaire que le §3.5.1 conduit **jusqu'à l'étage où le critère se place** : zone validée par `scope`, aucune exemption du §4.2 applicable. Ces conditions sont celles de toute validation : les répéter n'apprendrait rien, mais les taire rendrait ces critères faux pour une configuration pourtant supportée. Sous `scope.validateReplies: false`, un `Attention : le build casse` écrit en réponse de fil ne produit **aucun** diagnostic, et il en va de même sous un motif de `allowlistPatterns`, un auteur exempté ou un message généré par la plateforme — le verdict qu'un tel critère énonce ne vaut donc jamais qu'à son propre étage.

**Le périmètre d'activation (§6.2.3) est une précondition à part, et plus étroite.** Il ne conditionne que les résultats qui **bloquent ou publient un statut** : hors périmètre, « l'extension assiste sans jamais bloquer » (§6.2.1, condition 2), ce qui veut dire qu'elle **produit et affiche ses diagnostics comme d'ordinaire**. Étendre cette précondition aux critères purement diagnostiques — `CA-16`, `CA-17`, `CA-38` — autoriserait une implémentation à supprimer, hors périmètre, le retour visuel que le §6.2.3 exige justement d'y conserver.

Cette lecture ne s'étend **pas** aux critères dont le résultat ne vient pas de la validation d'un commentaire — bornes du plancher, épinglage, journalisation, retour arrière, rapport à blanc, ergonomie, dégradation. Ceux-là valent tels quels, y compris sur une PR dont tous les commentaires sont exclus, et une implémentation ne peut se prévaloir d'aucune des conditions ci-dessus pour y échouer.

Un critère qui dépend de l'une de ces conditions **autrement que par défaut** le dit explicitement : ainsi `CA-07`, qui repose sur l'exclusion des réponses, ou `CA-15` et `CA-01`, qui portent sur le périmètre. `CA-08`, `CA-20` et `CA-40` sont un cas distinct : ils éprouvent **une exemption déterminée**, dont l'application est précisément ce qu'ils mesurent — les **autres** exemptions du §4.2 doivent donc rester hors-jeu, sans quoi le mécanisme sous test pourrait être cassé sans que le critère s'en aperçoive.
...
- `CA-07` Une réponse dans un fil existant n'exige pas de label avec la configuration par défaut — `scope.validateReplies` y vaut `false`, et la zone est donc éteinte au sens du §3.5.1. C'est l'exception que le préambule annonce, pas une contradiction avec lui.
