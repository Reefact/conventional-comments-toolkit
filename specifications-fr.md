...
La règle est donc : lorsqu'un contournement rend **exactement** la configuration déjà en cache, il est enregistré comme **stérile**, et aucun autre n'est tenté pour ce dépôt avant l'expiration ordinaire du TTL. La seconde évaluation est alors inutile et n'a pas lieu — son verdict serait identique au caractère près. L'étranglement ne porte que sur les contournements stériles : un contournement fructueux n'est **jamais** étranglé, et n'a pas à l'être puisqu'il ne se reproduit pas. Une lecture `unreachable` n'est jamais tenue pour « la même » qu'une autre : ce n'est pas un état stable, et la compter comme telle étranglerait les contournements sur une panne passagère.
