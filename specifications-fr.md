...
La règle vaut pour tout motif de cette famille, et elle tient à ce qui distingue un préfixe nu d'une mention : `@notre-bot` est un jeton qui ne peut désigner qu'un compte, si bien que `^@notre-bot\b` peut rester ouvert sur ses arguments sans risque — c'est la grammaire même de `toolCommands`. `bors` est un mot ordinaire dans une phrase ; seule l'exigence que le corps **entier** soit la commande le rend discriminant.
